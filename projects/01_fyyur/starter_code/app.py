...
def search_venues():
  ...
  venueSearch = "%"+request.form.get('search_term')+"%"
  result = Venue.query.filter(Venue.name.ilike(venueSearch))
  #data = []
  #for row in result:
  #  UpcomingShows = Show.query.filter(Show.start_time >= datetime.now(), Show.venue_id == row.id)
  #  data += [{'id' : row.id, 'name' : row.name , 'num_upcoming_shows' : UpcomingShows.count()}]
  #upComingShows = Show.query.filter(Show.start_time >= datetime.now(), Show.venue_id == venue_id )
...
def show_venue(venue_id):
  ...
  venueInfo = Venue.query.filter_by(id = venue_id).first()
  pastShows = Show.query.filter(Show.start_time < datetime.now() , Show.venue_id == venue_id )
  upComingShows = Show.query.filter(Show.start_time >= datetime.now(), Show.venue_id == venue_id )
  result = {
    "id": venueInfo.id,
    "name": venueInfo.name,
...
def create_venue_submission():
  ...
  try:
    db.session.add(Venue(
        ...
        facebook_link=venueForm['facebook_link'],
        genres=venueForm.getlist('genres'),
...
def search_artists():
  ...
  result = Artist.query.filter(Artist.name.ilike(artistSearch))
  response = {"count" : result.count(), "data" : result }
...
def show_artist(artist_id):
  ...
  pastShows = Show.query.filter(Show.start_time < datetime.now() , Show.artist_id == artist_id )
  upComingShows = Show.query.filter(Show.start_time >= datetime.now(), Show.artist_id == artist_id )
  result = {
    "id": artistInfo.id,
...
def create_artist_submission():
  ...
  try:
    db.session.add(Artist(
        ...
        state=artistForm['state'],
        name=name,
        phone=artistForm['phone'],
        facebook_link=artistForm['facebook_link'],
        genres=artistForm.getlist('genres'),
        seeking_venue=False,
        website=""
    ))
    db.session.commit()
